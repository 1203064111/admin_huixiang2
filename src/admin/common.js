 ...
 */
var db_name="huixiang";
var is_debug=true;
var home_url='http://hx2-api.tstweiguanjia.com/';
var img_url='http://hx2-api.tstweiguanjia.com/';
var img_show='http://hx2-img3.tstweiguanjia.com/';
if(is_debug){
...
function changeFile(id,showId) {
    post(home_url+"/memberInterfaces/v1.0/getQiniuToken",{},function (data) {
        ...
        var subscription = observable.subscribe(function (response){
            ...
            // $(showId).attr('src', img_show+res.key);
            $(showId).attr('src',img_show+res.key);
        })
    })
}
...
function uploadRender2(upload,elem,id,doSuccess) {
    upload.render({
        elem: elem //绑定元素
        ,url: img_url+'/settingInterfaces/v1.0/uploadImg2' //上传接口
        ,accept: "file"
        ,size:'2048'
        ,before: function(obj){
            //预读本地文件示例，不支持ie8
        ...
        ,done: function(response){
            //上传完毕回调
            if (response.status === 'ok') {
                $(id).attr('src', img_show+response.data);
                if(doSuccess){
                    doSuccess(response.data);
                }
            } else if (response.status === 'error') {
...
function tableRender(table,elem,url,params,
    ...
    table.render({
        ...
        ,done: function(res, curr, count){
            var response =res;
            if (response.status === 'ok') {
                doSuccess(response.data,response.total,res, curr, count);
            } else if (response.status === 'error') {
                layer.msg(response.error, {
                    offset: '15px'
                    ,icon: 2
